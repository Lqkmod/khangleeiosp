...
def get_shortened_link(key):
    ...
    url_to_shorten = f'https://cc.lequockhang.site/key/key.php?key={key}'
    params = {
        "token": api_key,
        "format": "json",
        "url": url_to_shorten
...
def is_key_valid():
    try:
        with open(KEY_STATUS_FILE, "r") as f:
            ...
            saved_key = key_status.get("key")
            expiration_time_str = key_status.get("expiration_time")
...
if __name__ == "__main__":
    print("\033[1;34mLÊ QUỐC KHANG\033[0m")  # Màu xanh dương đậm
